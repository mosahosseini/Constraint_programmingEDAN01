...
public class Golomb3 {
     ...
     *
     * @param args the first argument specifies the number of marks, the second
     *             argument specifies the upper bound of the optimal solution.
     */
    public static void main(String args[]) {
        Golomb3 example = new Golomb3();
        example.model();
    }
    ...
    public void model() {
        ...
        for (int i = 1; i < noMarks; i++)
            for (int j = 0; j < i; j++)
                // store.imposePropagators(new XplusClteqZ(subs.get(index++), (noMarks - 1 - i +
                // j)
...
//        SplitSearch3 search = new SplitSearch3(store);
...
//        System.out.println("Total number of failed nodes = " + search.failedNodes);
        ...
        SelectChoicePoint<IntVar> select = new InputOrderSelect<IntVar>(store, numbers, new IndomainMin<IntVar>());
